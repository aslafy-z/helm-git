...
@test "should fail with bad prefix" {
    _run_helm_git "badprefix+https://github.com/jetstack/cert-manager@v0.5.2/contrib/charts/cert-manager/index.yaml"
    [ $status = 1 ]
}

@test "should fail with bad separator" {
    _run_helm_git "git+https://github.com/jetstack/cert-manager#v0.5.2/contrib/charts/cert-manager/index.yaml"
    [ $status = 1 ]
}
...
@test "should fail with bad protocol" {
    _run_helm_git "git+unknown://github.com/jetstack/cert-manager@master/index.yaml"
    [ $status = 1 ]
}

@test "should fail with no protocol" {
    _run_helm_git "git+git@github.com:toto/toto@master/index.yaml"
    [ $status = 1 ]
}

@test "should fail and warning with bad path and no ref" {
    _run_helm_git "git+https://github.com/jetstack/cert-manager@/contrib/charts/cert-manager/index.yaml"
    [ $status = 1 ]
